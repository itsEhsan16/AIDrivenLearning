...
if (!API_KEY) {
  throw new Error("Missing Google Gemini API Key. Please set VITE_GEMINI_API_KEY in your .env.local file.");
}
...
function cleanAndValidateRoadmapJson(text: string, expectedWeeks: number): any | null {
  ...
  if (openSquare > closeSquare) fixedJson += "]".repeat(openSquare - closeSquare);
  ...
  try {
    ...
    if (!roadmapData || !Array.isArray(roadmapData.weeks) || typeof roadmapData.title !== 'string') {
      return null;
    }
...
async function generateWeeksChunk(
  ...
  const previousTopics = Array.from(topicTracker.usedTopics).slice(-5).join(", ") || "None yet";
...
JSON format:
{
  ...
  try {
    ...
    console.log(`[RoadmapGen] AI response received for weeks ${startWeek}-${startWeek + numWeeks - 1}`);
    ...
    console.error(`[RoadmapGen] Error processing chunk response for weeks ${startWeek}-${startWeek + numWeeks - 1}:`, error);
    throw error;
  }
}
...
export const generateRoadmapWithAI = async (userGoals: UserGoals): Promise<Roadmap> => {
  try {
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
    const totalWeeks = parseTimeframeToWeeks(userGoals.timeframe);
    console.log(`[RoadmapGen] Timeframe: ${userGoals.timeframe} → Weeks: ${totalWeeks}`);
  ...
  "description": "Brief description of the entire learning path"
    ...
    for (let startWeek = 1; startWeek <= totalWeeks; startWeek += CHUNK_SIZE) {
      ...
      while (retries < MAX_RETRIES && !chunkSucceeded) {
        try {
          console.log(`[RoadmapGen] Generating chunk for weeks ${startWeek}-${startWeek + weeksInThisChunk - 1} (Attempt ${retries + 1})`);
          ...
          retries++;
          console.error(`[RoadmapGen] Attempt ${retries} failed for chunk starting at week ${startWeek}:`, error);
          if (retries < MAX_RETRIES) {
            const backoffTime = 1000 * (2 ** retries); // Exponential backoff: 2s, 4s
            console.log(`[RoadmapGen] Retrying in ${backoffTime / 1000}s...`);
            await delay(backoffTime);
          } else {
            console.error(`[RoadmapGen] All retries failed for chunk ${startWeek}. Using fallback.`);
          }
        }
      }
      ...
      if (!chunkSucceeded) {
        ...
        for (let week = startWeek; week < startWeek + weeksInThisChunk; week++) {
    ...
    console.log(`[RoadmapGen] Successfully generated roadmap with ${roadmap.tasks.length} tasks across ${roadmap.totalWeeks} weeks.`);
    return roadmap;

  } catch (error) {
    console.error("[RoadmapGen] Error in main generation, falling back to default roadmap:", error);
        ...
        topics: [
          {
            ...
            resources: ["https://developer.mozilla.org/en-US/docs/Web/HTML", "https://www.youtube.com/watch?v=pQN-pnXPaVg"]
          },
          {
            title: "Styling with CSS",
            desc: "Understand CSS syntax, selectors, and how to style HTML elements.",
            resources: ["https://developer.mozilla.org/en-US/docs/Web/CSS", "https://www.freecodecamp.org/learn/responsive-web-design/basic-css/"]
          },
          {
            title: "CSS Flexbox & Grid",
            desc: "Create responsive layouts using Flexbox and CSS Grid.",
            resources: ["https://css-tricks.com/snippets/css/a-guide-to-flexbox/", "https://cssgridgarden.com/"]
          },
          {
            title: "Accessibility Basics",
            desc: "Learn how to make web pages accessible to all users.",
            resources: ["https://www.w3.org/WAI/fundamentals/accessibility-intro/", "https://www.youtube.com/watch?v=3f31oufqFSM"]
          }
        ]
      },
      {
        ...
        topics: [
          {
            ...
            resources: ["https://javascript.info/", "https://www.youtube.com/watch?v=W6NZfCO5SIk"]
          },
          {
            title: "Functions & Loops",
            desc: "Write reusable code using functions and control flow with loops.",
            resources: ["https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Functions", "https://www.codecademy.com/learn/introduction-to-javascript"]
          },
          {
            title: "DOM Manipulation",
            desc: "Learn how to interact with web pages using the DOM API.",
            resources: ["https://developer.mozilla.org/en-US/docs/Web/API/Document_Object_Model/Introduction", "https://www.youtube.com/watch?v=0ik6X4DJKCc"]
          },
          {
            title: "Events in JS",
            desc: "Handle user input and browser events in JavaScript.",
            resources: ["https://javascript.info/events", "https://www.youtube.com/watch?v=XF1_MlZ5l6M"]
          }
        ]
      },
      {
        ...
        topics: [
          {
            ...
            resources: ["https://www.freecodecamp.org/news/es6-for-beginners/", "https://www.youtube.com/watch?v=NCwa_xi0Uuc"]
          },
          {
            title: "NPM & Modules",
            desc: "Learn about npm, installing packages, and using JS modules.",
            resources: ["https://docs.npmjs.com/about-npm", "https://www.youtube.com/watch?v=jHDhaSSKmB0"]
          },
          {
            title: "Debugging JS",
            desc: "Use browser dev tools and console to debug JavaScript code.",
            resources: ["https://developer.mozilla.org/en-US/docs/Learn/Common_questions/What_are_browser_developer_tools", "https://www.youtube.com/watch?v=H0XScE08hy8"]
          },
          {
            title: "APIs & Fetch",
            desc: "Work with APIs using fetch to get and display data.",
            resources: ["https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch", "https://www.youtube.com/watch?v=cuEtnrL9-H0"]
          }
        ]
      },
      {
        ...
        topics: [
          {
            ...
            resources: ["https://react.dev/learn", "https://www.youtube.com/watch?v=bMknfKXIFA8"]
          },
          {
            title: "JSX & Props",
            desc: "Learn JSX syntax and how to pass data with props.",
            resources: ["https://react.dev/learn/describing-the-ui", "https://www.youtube.com/watch?v=De-g4U0QZlI"]
          },
          {
            title: "State & Events",
            desc: "Manage component state and handle user events in React.",
            resources: ["https://react.dev/learn/state-a-components-memory", "https://www.youtube.com/watch?v=35lXWvCuM8o"]
          },
          {
            title: "Lists & Keys",
            desc: "Render lists of data and use keys in React.",
            resources: ["https://react.dev/learn/rendering-lists", "https://www.youtube.com/watch?v=c6bkr_udado"]
          }
        ]
      },
      {
        ...
        topics: [
          {
            ...
            resources: ["https://www.youtube.com/watch?v=pCA4qpQDZD8", "https://www.freecodecamp.org/news/react-projects-for-beginners/"]
          },
          {
            title: "Portfolio Website",
            desc: "Build and deploy a personal portfolio website using React.",
            resources: ["https://www.freecodecamp.org/news/portfolio-website-guide/", "https://www.youtube.com/watch?v=CK4gq9Fh2gA"]
          },
          {
            ...
            resources: ["https://testing-library.com/docs/react-testing-library/intro/", "https://www.cypress.io/", "https://www.youtube.com/watch?v=JKOwJUM4_RM"]
          },
          {
            title: "Performance Optimization",
            desc: "Optimize your app for speed and efficiency. Learn about code splitting, lazy loading, and performance profiling.",
            resources: ["https://react.dev/learn/optimizing-performance", "https://web.dev/fast/", "https://www.youtube.com/watch?v=CUxH_rWSI1k"]
          },
          {
            ...
            resources: ["https://vercel.com/docs", "https://www.netlify.com/with/react/", "https://www.youtube.com/watch?v=nhBVL41-_Cw"]
          },
          {
            ...
            resources: ["https://dev.to/", "https://github.com/", "https://www.youtube.com/watch?v=2Ji-clqUYnA"]
          }
        ]
      }
    ];
